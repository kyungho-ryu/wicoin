        ...
        if(result == true){
           console.log("AP 블록체인 등록 성공");
            ...
            if (database.db) {
                ...
                database.ApModel.findOne({'ssid':paramSsid}, function (err,ap) {
                    if(err){
                        return done(err);
                    }
                    if(ap){
                        console.log('기존 ssid가 있음');
                        res.writeHead('200', {'Content-Type': 'text/html;charset=utf8'});
                        res.write('<script>alert("기존 ssid가 존재합니다.");' +
...
// admin
var admin_listap = function (req, res) {
    console.log('ap 모듈 안에 있는 admin_listap 호출됨.');
        ...
        database.ApModel.list(options, function (err, results) {
            ...
            if (results) {
                ...
                database.ApModel.count().exec(function (err, count) {
                    ...
                    req.app.render('admin_listap', context, function (err, html) {
...
module.exports.admin_listap = admin_listap;
