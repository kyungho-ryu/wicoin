...
var mypage = function (req, res) {
    ...
    if (!req.user) {
        ...
        if(paramWriter == 'admin'){
            return res.redirect('/admin_listap');
        }else {
...
var user_drop = function (req, res) {
    console.log('mypage 모듈 안에 있는 user_drop 호출됨.');

    // URL 파라미터로 전달됨
    var paramUser = req.user._id;

    console.log('요청 파라미터 : ' + paramUser);

    var database = req.app.get('database');

    if (database.db) {

        database.UserModel.user_drop(paramUser, function (err, results) {
            if (err) {
                console.error('user 업데이트 중 에러 발생 : ' + err.stack);
...
module.exports.user_drop = user_drop;
